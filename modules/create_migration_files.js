#!/usr/bin/env node
module.exports = function create_db_data(path_to_dir, date_dir) {
	var fs  				 = require('fs'),
	...
	_.each(envs, function(env) {
		var read_dir     = path_to_dir + "/backup/" + env + "/" + date_dir
		fs.readdir(read_dir, function(err, files) {
			_.each(files, function(file) {
				if (file.match(env)) {

					read_file = path_to_dir + "/backup/" + env + "/" + date_dir	+ "/" + file
					...
					var entries 	 	 = _.compact( _.map( ascii_data.match(/^Started[\s\S]+?(?=^Started)/mg), function(matched){
						// ascii_data = ascii_data.match(/^Started[\s\S]+?(?=^Started)/mg).slice(1)
					...
					_.each(entries, function(entry) {
						...
						var key = "id_" + i;
						var h = hash(key, { "entry" : entry[0], "timestamp": entry[1], "error_status" : entry[2], "environment" : env});
